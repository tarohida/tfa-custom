...
ログイン後のリダイレクト先を管理画面から設定できます。

- **TFA 有効時**: TFA 検証成功後に設定したパスへリダイレクト
- **TFA 無効時**: 通常ログイン後も設定したパスへリダイレクト
- デフォルト: `<front>` (フロントページ)
- 設定例: `/user`, `/dashboard`, `/admin` など
- パスワードリセット時の動作は元のまま維持

**注意**: この設定は TFA の有無に関わらず、すべてのログインに適用されます。
...
- **リダイレクト制御**:
  - `hook_form_alter()` で通常ログインフォーム（`user_login_form`）と TFA エントリーフォーム（`tfa_entry_form`）にカスタム submit ハンドラーを追加
  - 共通のヘルパー関数 `_tfa_custom_set_redirect()` でリダイレクト処理を統一
